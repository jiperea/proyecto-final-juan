<!-- SPECKIT START -->
Plan activo: `specs/020-front-architecture/plan.md` (sin research/data-model/contracts — feature de gobernanza doc+lint, sin endpoints/IA/backend). **FE-6**: formaliza la arquitectura de front ya existente en `docs/front-architecture.md` (5 capas + 10 reglas (a)–(j) con nivel+justificación) y endurece eslint de forma proporcionada: 3 reglas `enforced` por baseline determinista (g sin default exports · b exhaustive-deps=error · j vistas no importan apiFetch) con **0 fixes de producción** (el front ya era conforme). Invariante conservador: **no toca lógica RBAC**. G1 ✅ PASS (8 pases; espiral cortada por simplificación — ver memoria `gate-remediation-spiral`) · G2 ✅ PASS (2 pases) · **G3 ✅ PASS** (impl+tests; tsc/eslint/stylelint/build + vitest 246/246 verdes; 0 bloqueantes). **Lista para PR a develop.** Historial: FE-5 (017-reskin) cerrada+merge; FE-1..4 (009/014/015/016) cerradas; M12 CI/CD cerrado; tooling de agentes dev-* + Playwright MCP mergeado (PR #18). Siguiente prevista: **FE-7 (021-front-dual-accent)** — doble token de acento (naranja vivo #DC5A24 donde no lleva texto + accesible en botones), verificación visual con Playwright MCP.
