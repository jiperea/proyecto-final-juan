...
describe('ops health/ready (FR-015, contrato)', () => {
  it('GET /health → 200 { status: ok }', async () => {
    const res = await request(buildApp(minimalAppDeps())).get('/health');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ status: 'ok' });
  });

  it('GET /ready → 200 ready cuando la BD responde', async () => {
    const res = await request(buildApp(minimalAppDeps())).get('/ready');
    expect(res.status).toBe(200);
  ...
  it('GET /ready → 503 not_ready cuando la BD no responde (fail-closed)', async () => {
    const res = await request(buildApp(minimalAppDeps({ checkDb: async () => false }))).get('/ready');
