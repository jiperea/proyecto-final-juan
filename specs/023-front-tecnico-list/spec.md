...
- Q: ¿Cómo se muestra el «técnico» en la tarjeta? → A: **Condicional, no fijo**: «Tú» **solo si** `assigned_to` coincide con el usuario en sesión; si no coincide, su **identificador**; si es nulo, representación **neutra**. Renderizar «Tú» incondicionalmente enmascararía un fallo de aislamiento en backend (IDOR) → la UI refleja el dato real, no lo asume.
...
- Q: ¿Qué muestran los tiles de evidencia? → A: Etiqueta **neutra derivada de `content_types`** (p. ej. «Imagen N», «PDF N», o «Adjunto N» si el tipo no se reconoce), **1-based**; NO «foto» fija (asumiría que todo adjunto es imagen). Es un marcador del recuento, sin imagen real (contrato sin URL firmada).
...
2. **Given** una orden con evidencia `count` = N > 0, **When** se renderiza la sección, **Then** se muestran **N tiles** 4/3 numerados 1-based con etiqueta derivada de `content_types` (no «foto» fija); con `count` = 0, se muestra «sin evidencia» y 0 tiles.
...
- **FR-002**: THE fila de meta SHALL mostrar el **cliente** (o **«—»** si el contrato no lo expone) y el **técnico**, resuelto **condicionalmente**: **«Tú»** si `assigned_to` == usuario en sesión; el **identificador** de `assigned_to` si no coincide; representación **neutra** si es nulo. La UI **no asume** que toda orden del listado sea del usuario (refleja el dato, no enmascara un posible fallo de aislamiento en backend).
...
- **FR-006**: WHEN el payload del detalle incluye `evidence` con `count` = N (>0) THE front SHALL renderizar **N tiles placeholder** de relación **4/3** (`--radius-sm`), numerados **1-based**, con etiqueta **derivada de `content_types`** (p. ej. «Imagen 1», «PDF 2»; «Adjunto N» si el tipo no se reconoce) — **sin** asumir «foto» ni construir imágenes reales (el contrato no expone URL firmada). WHEN `count` = 0 THE front SHALL mostrar un **estado explícito «sin evidencia»** y **0** tiles (comportamiento único, sin ocultar la sección en silencio).
