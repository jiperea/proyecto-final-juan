...
2. **Given** el listado (acotado server-side a órdenes propias), **When** se muestra cada tarjeta, **Then** el técnico aparece como **«Tú»** (flujo esperado: `assigned_to == userId`). **[Guarda defensiva, unit]** Given una orden fabricada con `assigned_to != userId`, **Then** se muestra su **UUID truncado** (nunca «Tú»); con `null`, representación neutra.
...
- **Meta sin datos**: cliente ausente (contrato) → «—»; `assigned_to` == usuario → «Tú»; `assigned_to` distinto → **UUID truncado** (8 chars); `assigned_to` `null` o sesión sin resolver → literal **«Sin asignar»** (nunca «Tú» ni UUID).
...
- **FR-002**: THE fila de meta SHALL mostrar el **cliente** — siempre **«—»** (el contrato no expone cliente) — y el **técnico**, resuelto **condicionalmente** sobre `Order.assigned_to` (`uuid | null`): **«Tú»** si `assigned_to` == `userId` de la sesión; el **UUID truncado** (8 chars, mono) de `assigned_to` (identificador opaco; el contrato no expone nombre) si está presente y no coincide; el literal **«Sin asignar»** si es `null` **o** mientras el `userId` no esté resuelto (sesión cargando) — **nunca** el UUID en ese instante, para no rotular mal. Regla válida en **cualquier** vista que reutilice la tarjeta: en la del técnico es «Tú»; en supervisor/oficina, el UUID del asignatario. El `userId` de la sesión se obtiene del **contexto de sesión ya existente** (`useSession`, consumido de solo-lectura; sin cambios en el módulo de auth). **Alcance del listado (contrato `listOrders`, verificado)**: el backend **acota server-side por rol** (`orderScopeFor`); el técnico recibe **solo sus propias** órdenes activas (`assigned`/`in_progress`/`pending_review`; `draft`/`closed` nunca aparecen). Por tanto, en esta vista `assigned_to == userId` **siempre** → «Tú» es el render esperado; **no llega dato ajeno al cliente** (sin filtrado de aislamiento en cliente). Las ramas **UUID/neutra** son una **guarda defensiva**: si el backend devolviera algo inesperado, la UI muestra el **UUID truncado** (8 chars, mono, sin desbordar a ≤390px) en vez de rotular falsamente «Tú» — aflora la anomalía, no la enmascara (unit-testable con prop fabricada; no es un flujo de aceptación).
...
- **FR-010**: THE **listado y el detalle** SHALL seguir siendo **server-authoritative**: el listado (`listOrders`) devuelve solo las órdenes del alcance del rol (técnico = propias activas; sin filtrado de aislamiento en cliente), y el detalle se obtiene por su propia consulta por `id` con el control existente (401/403/404 para órdenes fuera del rol/ámbito). Esta feature **no** relaja ese control ni renderiza datos que el backend no devuelva para el rol. Las **notas** y cualquier dato de cliente se muestran **escapados** y **solo** si el backend los entrega al rol (misma exposición que hoy; sin nueva superficie de PII); no se añaden logs de esos datos.
