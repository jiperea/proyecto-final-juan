...
- [ ] T013 Catch-all de errores de BD en `backend/src/handlers/error-mapper.ts` (D-10, conforme FR-010): **todo** error de BD ≠ FK-de-`to_assignee` (deadlock, timeout, **BD no disponible**, UUID no atrapado, constraint futura) → **500** genérico `{code,message,agent_action}` sin filtrar SQLSTATE/constraint/columna/query. El `P2003` se mapea a `INVALID_ASSIGNEE`(422) **sólo** si `meta.field_name`/constraint es la de **`to_assignee`**; `P2003` de `actor_id`/`from_assignee` → **500** (P4). **NO** hay 503 (reconciliación diferida a BL-066).
...
- [ ] T016 [P] [US1] (Red) Integración happy path + auditoría `backend/tests/integration/reassign-order.spec.ts`: 200, `assigned_to=T2`, `status` intacto (assigned/in_progress), `version`+1, **1** fila `event_type=reassignment` con `from_assignee/to_assignee/reason` y `from_status==to_status==` estado real (assigned→assigned, in_progress→in_progress); incluye **orden huérfana** (`assignedTo=null`→`from_assignee=NULL`). (SC-001; escenarios 1/2/11; P1)
...
- [ ] T018 [P] [US1] (Red) Integración no-enumeración `backend/tests/integration/reassign-order-notfound.spec.ts`: 404 **byte-idéntico** (cuerpo+cabeceras, `code`=`ORDER_NOT_FOUND`, sin `details`) entre las **cuatro** vías — inexistente, existente-no-reasignable (`ORDER_NOT_REASSIGNABLE` colapsado), colapso post-UPDATE (escenario 9) y **`orderId` malformado** (no uuid; interpretación conforme de FR-004, P5). (SC-003/SC-008, FR-002/FR-004, D-04/D-14)
...
- [ ] T024 [P] [US1] (Red) Integración errores de BD `backend/tests/integration/reassign-order-errors.spec.ts`: (a) error de BD ≠ FK-`to_assignee` (incl. **BD no disponible**, y **fallo durante la consulta de visibilidad**) → **500** genérico sin SQLSTATE/constraint/columna/query (no `null`→404, P6); (b) `P2003` de `actor_id`/`from_assignee` → **500**, no 422 (P4). (SC-007, FR-010, D-10)
...
- [ ] T029 [US1] Adaptadores en `backend/src/infra/repositories/order-write-side-repository.ts`: `OrderVisibilityPort` (consulta única `WHERE id AND status IN {assigned,in_progress}` → snapshot con status); `OrderReassignmentPort.reassign` usando el helper privado `conditionalWriteWithAudit` (dentro de `$transaction`: relectura within-tx → UPDATE `assigned_to=T2, version+1` condicional → si count=1, **ensambla e inserta** la fila de auditoría `reassignment` con `from_status=to_status=`status releído, `from_assignee`=antiguo, `to_assignee`=T2, `actor_id`); devuelve `{count, order|null}` **sin clasificar** (P1/N2). `P2003` → `INVALID_ASSIGNEE` **sólo** si la constraint es la de `to_assignee` (inspecciona `meta.field_name`); otras FK → propaga a 500 (P4). `UserLookupPort` sobre `user-repository`/Prisma. (depende de T027)
