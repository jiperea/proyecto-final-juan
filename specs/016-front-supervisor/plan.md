...
- **Nuevo (FE-4)**:
  ...
  - **Capa api**: `reviewRequestSchema` (Zod: `decision` enum, `reason` 1..1000 code points ≥1 imprimible, opcional pero exigido en reject en el componente) + `reviewOrder(orderId, body)` y `summarizeIncident(orderId)` en `write-api.ts` (**`summarizeIncident` = wrapper de cliente del `operationId` del contrato `summarizeOrderIncident`; alias 1:1, mismo endpoint `POST /orders/{id}/ai-summary`**). Mutaciones `useReview(orderId)` (invalida `['order',id]`+`['orders']`; onError 404→invalida) y `useSummary(orderId)` (POST bajo demanda; **descarta respuestas fuera de orden**; 401→refresh sin auto-retry).
