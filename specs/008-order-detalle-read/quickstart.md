...
| 12 | GET concurrente con `submitOrderExecution` en vuelo | `GET .../{id}` | motivo+notas del **mismo** ciclo (snapshot consistente) | FR-003, D4 |
| 13 | Reasignación T1→T2 de orden rechazada sin atender | T2 `GET .../{id}` | `200`: T2 ve motivo + notas/evidencia del ciclo anterior; T1 (ex-dueño) → `404` | FR-005, S-003 |
...
| 15 | Cualquier 401/404 | — | registro append-only de acceso denegado con `recurso` saneado (UUID o `<malformed>`) | FR-009 |
...
| 17 | Contrato | `GET .../{id}` | conforme a `OrderDetailResponse`; `evidence`/`notes` opcionales; dispatcher los omite | FR-008 |

## Criterio de "hecho" (para G3)

- Todos los escenarios 1–17 verdes; contract test 100% (5 códigos); cobertura dominio ≥80% y servicios ≥80%.
