...
- [ ] T010 [P] [US1] (Red) Contract test de `getOrderDetail` en `backend/tests/contract/get-order-detail.contract.spec.ts`: validación **estricta** contra el JSON Schema (ajv `strict`, `additionalProperties:false`) → 200 conforme a `OrderDetailResponse` (technician con motivo) **sin campos extra** (ningún campo de auditoría adicional); `evidence`/`notes`/`last_rejection_reason` opcionales; 401/404/500/503; **nunca 403**.
...
- [ ] T017 [US1] Implementar el handler `backend/src/handlers/orders/get-order-detail.ts` (rama technician): precedencia **401→404**; `orderId` malformado → 404; mapeo de errores `{code,message,details,agent_action}`; invoca `DeniedAccessLoggerPort` en 401/404; ensambla vía dominio (depende de T015, T016, T006).
...
- [ ] T031 [P] (Red→Green) Test propio de **SC-005** (mínimo privilegio XI) en `backend/tests/integration/get-order-detail.min-privilege.spec.ts`: no hay vía para pedir otra transición/orden/registro (query params `?auditId=`/`?history=` u otras rutas se ignoran/404); la respuesta **nunca** incluye más de un campo de auditoría (`last_rejection_reason`); orden ajena→404 no expone su motivo.
- [ ] T032 Actualizar `docs/traceability.md` (FR-001..FR-009 + SC-005 → `getOrderDetail` → T0xx → tests).
- [ ] T033 Verificar cobertura (dominio ≥80%, servicios ≥80%, 100% contrato y ramas de visibilidad/404), `tsc`/`eslint` limpios, y ejecutar los 18 escenarios de `quickstart.md`.
