...
- **FR-017**: THE evidencia visible SHALL ser la del **ciclo vigente** = el del **`attempt`/`auditId` más reciente enviado**; `getOrderDetail.items`/`getOrderEvidence` exponen solo ese conjunto. WHEN una orden rechazada se reenvía, el **nuevo submit** crea las filas del nuevo `attempt`; **en esa misma transacción de submit** los binarios del ciclo anterior se **purgan** (sus `evidenceId` → 410 a autorizados). El reemplazo ocurre **en el submit** (no antes) → si el técnico abandona el reintento, la evidencia del ciclo rechazado **sigue disponible** hasta que haya un nuevo submit.
