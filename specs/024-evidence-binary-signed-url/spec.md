...
- **FR-023**: THE `object_ref` staged que devuelve `uploadOrderEvidence` SHALL estar **ligado a (técnico dueño que lo subió, `orderId`)**. WHEN `submitOrderExecution` incluye `object_ref`s en su `evidence[]` THE backend SHALL **re-verificar bajo la concurrencia optimista de la orden (`version`/If-Match, ya existente)** que cada `object_ref` fue staged por el **mismo técnico dueño actual y para esa orden**, en la **misma transacción** que la transición; una ref ajena/de otra orden/de otro actor → **rechazo (404 uniforme / 422)** sin crear fila. **TOCTOU con reasignación**: una reasignación concurrente sube `version` → el submit del dueño saliente falla con **409** (rancio) y no crea evidencia.
- **FR-024**: THE GC de blobs SHALL ser **un único proceso programado** que purga **todo blob sin fila `OrderEvidence`** con antigüedad **> TTL de staging (24 h)** — cubre por igual el staged abandonado y el huérfano por rollback (mismo estado: blob-sin-fila). Un blob que sí tiene fila (committeado en un submit) no se purga por este GC (lo rige la retención de 90 días, FR-018). El TTL distingue **abandonado/huérfano** (>24 h, sin fila) de **en vuelo** (reciente).
