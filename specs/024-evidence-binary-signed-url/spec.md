...
### Session 2026-07-16c (remediación G1 r8 — simplificación anti-espiral, Principio XV)

- **Retención de `closed` = purga sin acceso (S-008)**: el contrato fija `closed` **fuera de alcance de todo rol** en `getOrderDetail` (→404); como la autz de evidencia hereda esa regla (FR-003), la evidencia de una orden `closed` **no se sirve nunca** (404, sin regla RBAC nueva). La retención de 90 días es un **ciclo de almacenamiento puro**: el binario se **purga físicamente**, sin semántica de acceso. El **410** queda **solo** para el caso «actor autorizado sobre orden **en alcance** (estados activos) cuya fila existe pero el blob es legacy/superado». SC-006 asevera la **purga física del blob**, no una respuesta 410 de `closed`.
- **Un `object_ref` ↔ una fila (H-001)**: el `submit` **solo** acepta `object_ref`s en **staging puro** (sin fila `OrderEvidence` previa); un ref que ya tiene fila (p. ej. reenvío que reusa fotos del ciclo rechazado) → **422 «vuelve a subir»**. Cada blob pertenece a **exactamente una** fila → el GC de una fila superada nunca borra un blob compartido con la vigente.
- **TTL de staging autoritativo por edad (H-002)**: el rechazo por TTL en el `submit` se decide por la **edad del staging** (timestamp), evaluada **dentro de la transacción**, con independencia de si el GC ya corrió (>24 h → 422 aunque el blob exista aún). El `submit` re-valida la **existencia del blob en su propia transacción**; el GC nunca purga un blob referenciado por una fila que commitea → no hay fila-vigente-sin-blob tras un submit exitoso.
- **Doble-submit idempotente (H-003)**: la concurrencia optimista sobre `version` garantiza que **un solo** submit del mismo dueño commitea (crea attempt/filas); el segundo lee `version` obsoleta → **409** (o 404 si hubo reasignación). No se crean dos attempts por doble clic.
- **Cifrado en reposo verificable (T-001/SC-004)**: **AES-256-GCM**; el test lee los **bytes crudos** del adaptador de almacenamiento (bypaseando el descifrado) y afirma que **difieren byte a byte** del binario original subido.
- **Cota de latencia de purga (T-002)**: el job de purga (FR-018) y el GC (FR-024) corren **al menos a diario** → latencia máxima ≤ 24 h tras cumplir el umbral; el test fija ese instante de corte.
- **Auditoría de accesos denegados (M-AUDIT/S-010)**: `getOrderEvidence`/`uploadOrderEvidence` **heredan** la señal best-effort de acceso denegado (401/404) ya establecida en `getOrderDetail` (contrato #009) — patrón existente, no maquinaria nueva.
- **Endurecimiento del binario servido (S-009)**: la respuesta 200 añade `X-Content-Type-Options: nosniff` y `Content-Type` estricto **derivado del magic-byte real** (no del declarado), defensa en profundidad frente a polyglot sobre FR-019.
...
- **Evidencia de orden `closed`**: **no se sirve a ningún rol** (404, `closed` fuera de alcance de `getOrderDetail`, FR-003/FR-007); no hay ventana de lectura. La retención de 90 d purga el binario en segundo plano (almacenamiento puro), sin ruta de acceso que devuelva 410.
- **Evidencia legacy/superada en orden activa**: para un actor autorizado, la fila existe pero el blob es legacy (nunca almacenado) o fue superado → **410 «no disponible»** sin filtrar detalles.
...
- **FR-004**: WHEN la autorización es válida THE sistema SHALL servir el binario **same-origin en la misma respuesta autenticada por sesión** (200), con `X-Content-Type-Options: nosniff` y un `Content-Type` estricto **derivado del magic-byte real** (no del declarado por el cliente), como defensa en profundidad frente a polyglot (complementa FR-019); **no** entrega al cliente ninguna URL firmada ni token detached. La **URL firmada con TTL ≤ 300 s** existe **solo internamente** (backend ↔ adaptador de almacenamiento) para leer el objeto; **nunca** se expone al cliente. Cada petición del cliente se **re-autoriza por sesión** (no hay capacidad portadora que viaje).
...
- **FR-007**: THE comprobación SHALL seguir esta **precedencia**: (1) sin sesión → **401**; (2) autorización (orden+rol, como `getOrderDetail`) → si no autorizado / orden inexistente / ajena / **`closed`/`draft` (fuera de alcance de todo rol)** / `evidenceId` inexistente → **404 uniforme** (nunca 403), respuesta **constante** e indistinguible; (3) **solo si el actor está autorizado sobre una orden en alcance** se evalúan existencia del binario/legacy/superado (410, FR-009). Así **410 nunca es visible a un no-autorizado** ni para `closed` (evita enumerar «existió y fue purgada» vs «no existe»). Todo acceso denegado (401/404) emite la **señal best-effort de acceso denegado** heredada de `getOrderDetail` (recurso opaco saneado, sin PII; #009).
...
- **FR-009**: THE binario de evidencia SHALL retenerse mientras la orden no esté `closed` y **hasta 90 días tras el cierre**; superado ese plazo se **purga físicamente** (ciclo de almacenamiento puro, sin semántica de acceso). **La evidencia de una orden `closed` no se sirve a ningún rol** (404 por autz heredada de `getOrderDetail`, FR-003; `closed` está fuera de alcance de todo rol) — no hay ventana de lectura de `closed`. El **410 «no disponible»** se reserva para un actor **autorizado** sobre una orden **en alcance** (estados activos) cuya **fila existe pero el blob es legacy** (evidencia previa a esta feature, solo metadatos) **o fue superado** (FR-017); metadatos y auditoría permanecen (inmutables, XI). El **410 solo se expone a autorizados** (FR-007).
...
- **FR-023**: THE `object_ref` staged que devuelve `uploadOrderEvidence` SHALL estar **ligado a (técnico dueño que lo subió, `orderId`)** y ser **staging puro** (sin fila `OrderEvidence` previa). WHEN `submitOrderExecution` incluye `object_ref`s en su `evidence[]` THE backend SHALL **re-verificar bajo la concurrencia optimista de la orden (`version`/If-Match, ya existente)**, en la **misma transacción** que la transición, que cada `object_ref`: (a) fue staged por el **mismo técnico dueño actual y para esa orden**, (b) **no tiene fila ya creada** (no se reusa evidencia committeada de otro `attempt`), y (c) **su blob existe** (re-lectura dentro de la transacción). **Códigos deterministas**: `object_ref` **ajeno/de otra orden/de otro actor** → **404 uniforme** (no-enumeración, como FR-007); `object_ref` **malformado** → **422**; `object_ref` **con fila ya existente** (reuso de evidencia committeada) → **422 «vuelve a subir»**; `object_ref` **propio válido pero cuya edad de staging supera el TTL** (24 h, medida por timestamp **con independencia de si el GC ya corrió**) → **422 «evidencia expirada, vuelve a subir»**; **`object_ref` repetido dentro del mismo `evidence[]`** (mismo ref dos veces en un submit) → **422** (entrada inválida; no se deduplica en silencio, coherente con el carácter exhaustivo/determinista de estos códigos). El tope ≤10 (FR-022) se cuenta sobre el **array crudo** `evidence[]`. Ningún caso crea fila; **cada blob committeado pertenece a exactamente una fila** (habilita el GC seguro de FR-024). **TOCTOU con reasignación**: una reasignación concurrente sube `version`; el submit del dueño **saliente** → **404 uniforme** (coherente con FR-007/020; no 409, para no revelar el cambio de estado). **Doble-submit del mismo dueño**: la concurrencia optimista sobre `version` garantiza que **solo uno** commitea (crea attempt/filas); el segundo lee `version` obsoleta → **409** (no crea un segundo attempt).
- **FR-024**: THE GC de blobs SHALL ser **un único proceso programado** que purga todo blob **sin fila `OrderEvidence` vigente** — es decir: (a) sin fila (staged abandonado >24 h TTL, o huérfano por rollback), y (b) con fila marcada **superada** por un reenvío (FR-017). Un blob cuya fila es **vigente y committeada** no lo toca este GC (lo rige la retención de 90 días, FR-018). Como **cada blob pertenece a exactamente una fila** (FR-023: no hay refs compartidos entre attempts), purgar el blob de una fila superada **no puede** afectar a una fila vigente. El GC corre **al menos a diario** → latencia máxima de purga de staging ≤ 24 h tras el TTL. Así se cubre también la purga física del ciclo superado (cierra el hueco de que «con fila» quedara sin GC).
...
- **FR-017**: THE evidencia visible SHALL ser la del **ciclo vigente** = el del **`attempt`/`auditId` más reciente enviado**; `getOrderDetail.items`/`getOrderEvidence` exponen solo ese conjunto. WHEN una orden rechazada se reenvía, el técnico **vuelve a subir** las fotos del nuevo ciclo (no se reusa `object_ref` committeado — FR-023.b); la **transacción PG del nuevo submit** crea las filas del nuevo `attempt` con **blobs propios** y **marca superado** el ciclo anterior (el reemplazo **lógico** —qué se expone— es inmediato al commit; los `evidenceId` superados → **410** a autorizados sobre orden en alcance). Como cada blob pertenece a una sola fila, purgar el blob del ciclo superado **nunca** afecta al vigente. La **purga física** de los binarios superados la hace el **GC de FR-024** (que cubre blobs de fila superada), fuera de la transacción PG (coherente con FR-011). Si el técnico abandona el reintento (sin nuevo submit), la evidencia del ciclo rechazado **sigue vigente**.
- **FR-018**: THE purga por retención (FR-009) SHALL ejecutarse por un **proceso programado** (no perezoso) que corre **al menos a diario** (latencia máxima entre cumplir el umbral de 90 días y la purga efectiva ≤ 24 h) y recorre órdenes `closed` con antigüedad > 90 días purgando su binario (un binario nunca accedido igualmente se purga). Es independiente de la purga inmediata de ciclos superados (FR-017).

### Key Entities

- **OrderEvidence** (modelo existente verificado, `prisma/schema.prisma`): fila por evidencia con `id` (uuid), `orderId`, **`objectRef`** (ref opaca al blob; es **client-visible solo en el flujo upload↔submit** —ya viaja en el body actual de `submitOrderExecution`— pero **nunca en logs ni en respuestas de lectura/detalle**, donde solo se expone `evidenceId`), `contentType` (allowlist), `sizeBytes`, `uploadedBy`, **`attempt`** y **`auditId`** (enlace a la `OrderAudit` del envío). El **ciclo de ejecución** se discrimina por `attempt`/`auditId` (ya en el modelo); el «ciclo vigente» es el del `attempt`/audit más reciente. Una fila `OrderEvidence` existe **solo tras el submit**; un **blob staged** (subido y aún sin submit) es solo un objeto en almacenamiento **sin fila** (no direccionable). **Cada blob committeado pertenece a exactamente una fila** (`objectRef` no se comparte entre `attempt`s: un reenvío re-sube blobs propios, FR-023.b), lo que hace seguro el GC por fila (FR-024). El binario vive en el almacenamiento (infra), cifrado; el dominio maneja metadatos + puerto de almacenamiento.
...
| FR-002 | `uploadOrderEvidence` | `contract/upload-evidence.contract` + `integration/evidence-content-validation` (415 tipo, 413 tamaño, 422) |
| FR-003 | `getOrderEvidence` | `integration/evidence-authz` (dueño/supervisor sí, dispatcher no) |
| FR-004 | `getOrderEvidence` | `contract/get-evidence.contract` (200 same-origin por sesión; 0 URL/token cliente-visible en cuerpo/headers) |
...
| FR-009 | (retención) | `integration/evidence-retention-purge` (purga física del blob de `closed`; acceso a evidencia de `closed` → **404** para todo rol, nunca 410) |
| FR-010 | front detalle | `frontend OrderDetailView.evidence.test` (abrir, carga/error) |
| FR-011 | `uploadOrderEvidence` | `integration/evidence-atomic-gc` (commit BD = verdad; huérfano purgado) |
| FR-012 | `uploadOrderEvidence` | `contract/upload-evidence.contract` (submit sigue JSON, sin romper) |
| FR-013 | front detalle | `frontend OrderDetailView.evidence.test` (blob:, sin URL en DOM/Referer) |
| FR-014 | `getOrderDetail` | `contract/detail-evidence-items` (evidenceId+content_type) |
| FR-015 | `getOrderEvidence` | `integration/evidence-404-uniforme` (evidenceId∉order → 404) |
...
| FR-018 | (job de purga) | `integration/evidence-retention-purge` (compartido con FR-009) |
...
- **SC-004**: La evidencia **no** es accesible por URL directa sin autorización (test negativo) y está **cifrada en reposo con AES-256-GCM**: el test lee los **bytes crudos** del adaptador de almacenamiento (bypaseando el descifrado) y afirma que **difieren byte a byte** del binario original subido (no basta un flag de configuración).
- **SC-005**: **0** apariciones de URL firmada / `object_ref` / binario en logs (grep de logs en test).
- **SC-006**: La **retención** del binario PII se aplica: tras el plazo (90 días post-cierre, latencia de purga ≤ 24 h) el binario se **purga físicamente** — el test verifica la **ausencia del blob en el almacenamiento** (ciclo de almacenamiento puro). La evidencia de una orden `closed` **no se sirve** en ningún momento (404, coherente con `getOrderDetail`); el **410** se prueba solo para el caso legacy/superado sobre orden en alcance.
...
- **Acceso a órdenes `closed` durante la retención (≤90 d)**: **ningún rol** accede a la evidencia de una orden `closed` (el contrato pone `closed` fuera de alcance de `getOrderDetail` para todo rol → 404, y FR-003 hereda esa regla exacta). La retención de 90 d es un **ciclo de almacenamiento puro** (purga física del blob), sin ruta de lectura ni respuesta 410 de acceso. No se especifica una regla RBAC nueva para `closed` (Principio XV; decisión r8/S-008).
