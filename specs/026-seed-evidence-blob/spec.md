...
- **`docker compose down -v`**: vacía el volumen de Postgres pero **no** el bind-mount de `EVIDENCE_STORAGE_DIR`; NO es un reset completo. Tras `down -v` + `make up`, la evidencia ancla sigue sirviéndose porque su `object_ref` es fijo y su blob se sobrescribe (FR-007); para un almacén limpio (sin blobs de sesiones previas) se usa **`make reset`**. Se documenta para no confundir `down -v` con un reset total.
...
- **FR-007**: THE seed SHALL usar para la evidencia ancla un `object_ref` **determinista y estable entre ejecuciones** —el que devuelve `StoragePort.putStaged(...)` para la **imagen fija embebida** (content-addressed por hash del contenido), en lugar del placeholder literal actual del seed— de modo que re-sembrar la misma imagen **produzca el mismo `object_ref` y sobrescriba el mismo blob** (idempotente, sin acumular huérfanos aunque la BD se vacíe por otras vías como `docker compose down -v`, que no toca el bind-mount del almacén). La fila `OrderEvidence` sembrada usa ese `object_ref` devuelto (no el placeholder). Mantiene el invariante de 024 «un `object_ref` ↔ una fila».
...
| FR-003 | — | (pend. tasks) | `should abortar (exit!=0, mensaje nombra EVIDENCE_ENC_KEY+acción) si falta/inválida la clave` |
| FR-004 | — | (pend. tasks) | `should abortar con NODE_ENV=production o host externo (match exacto; rechaza evil-db.example.com) sin filtrar DATABASE_URL/credenciales` |
...
| FR-007 | — | (pend. tasks) | `should usar object_ref fijo (re-sembrar sobrescribe el mismo blob, sin acumular)` |
...
| FR-010 | — | (pend. tasks) | `should ser atómico: interrupción revierte la BD (vacía), blob antes que fila; poblada⟺completa` |
| FR-011 | — | (pend. tasks) | `make reset (todo en contexto backend) limpia BD+EVIDENCE_STORAGE_DIR y re-siembra` · `guard falla ⇒ prisma migrate reset NUNCA se invoca (orden, no solo exit≠0)` · `dir ausente ⇒ paso (b) no falla (idempotente)` · `seed sobre BD no vacía fail-fast con RESEED_HINT (throw sin cambios; test 019 usa la constante)` |
| FR-013 | — | (pend. tasks) | `config de la app y el seed usan la misma función de validación de la clave` |
...
- **SC-001**: **Inmediatamente tras** re-sembrar la BD de desarrollo navegada (antes de cambiar el estado de la orden ancla), la evidencia de esa orden se sirve con **200** (no 410) al solicitarla, en el **100 %** de las ejecuciones del seed, para técnico dueño y supervisor.
