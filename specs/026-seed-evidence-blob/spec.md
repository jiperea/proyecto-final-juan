...
- **FR-007**: THE seed SHALL usar para la evidencia ancla un `object_ref` **fijo y constante** (documentado, p. ej. `seed-anchor-evidence`), de modo que re-sembrar **sobrescriba el mismo blob** (idempotente, sin acumular huérfanos aunque la BD se vacíe por otras vías como `docker compose down -v`, que no toca el bind-mount del almacén). Mantiene el invariante de 024 «un `object_ref` ↔ una fila».
