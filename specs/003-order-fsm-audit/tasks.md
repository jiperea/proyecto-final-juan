...
- [X] T001 Añadir modelo Prisma `OrderAudit` (id UUID v7, order_id FK→Order `onDelete: Restrict`, actor_id
...
- [X] T002 [P] **[Red]** Unit FSM `isLegalTransition(from,to)` exhaustivo — legales (assigned→in_progress,
...
- [X] T003 FSM: tabla de transiciones + `isLegalTransition` (sin `draft→assigned`) —
  `backend/src/domain/order/transition-table.ts` (FR-001, D1)
- [X] T004 [P] Errores de dominio en el catálogo `ErrorCode` + error-mapper de 001:
...
- [X] T005 [P] Puerto `OrderTransitionPort.applyTransition({orderId,toStatus,actorId,reason?,expectedVersion,guard?})`
...
- [X] T006 [P] [US1] **[Red]** Unit `applyTransition` (use case) con puerto fake: transición legal delega en el
...
- [X] T007 [P] [US1] **[Red]** Integration (Postgres real) transición legal **parametrizada sobre las 4
...
- [X] T008 [P] [US1] **[Red]** Integration efectos negativos SIN efecto, en **orden determinista**: `orderId`
...
- [X] T009 [P] [US1] **[Red]** Integration **atomicidad** (SC-004): `actor_id` inexistente → FK falla dentro de
...
- [X] T010 [P] [US1] **[Red]** Integration **append-only** (SC-003): un `UPDATE`/`DELETE` directo sobre
...
- [X] T011 [P] [US1] **[Red]** Integration **concurrencia** (SC-002): dos `applyTransition` con la **misma
...
- [X] T012 [P] [US1] **[Red]** Integration **guarda de pertenencia** (SC-005): `guard.assignedTo` que no
...
- [X] T013 [P] [US1] **[Red]** Integration **no-fuga de `reason`** (SC-006): forzar un error con un `reason`
...
- [X] T014 [P] [US1] **[Red]** Arquitectura (FR-006): `domain/order` no importa infra + **`status`/`version`
...
- [X] T015 [US1] Use case `applyTransition` (dominio): valida `isLegalTransition`; delega en el puerto; devuelve
  `Result` — `backend/src/domain/order/apply-transition.ts` (FR-002/006/007)
- [X] T016 [US1] Repo `PrismaOrderTransitionRepository` con `$transaction`: **UPDATE condicional**
...
- [X] T017 [US1] Extender redacción de logs a `reason` **y a errores de BD** (nunca `reason` ni el error crudo
  de Postgres en logs/`details`/`agent_action`) — `backend/src/infra/logger.ts`,
  `backend/src/handlers/error-mapper.ts` (FR-008, SC-006)
- [X] T018 [US1] Wiring: `OrderTransitionPort` en container (no se monta ruta — dominio puro) —
...
- [X] T019 [P] Actualizar `docs/traceability.md` con matriz RF→tarea→test de 002b; **FR-009** (no-enumeración) y
...
- [X] T020 Validación `quickstart.md` (migrate+seed+test verde contra Postgres real) **+ verificar que el `down`
